...
namespace CompactFormatter.Tests
{
	...
	public class CustomSerializableObject : ICSerializable
	{
		...
		public CustomSerializableObject(int a, Decimal f, String s)
		{
			this.a = a;
			this.f = f;
			this.s = s;
	...
	public class CustomSerialization
	{
		...
		[Test]
		[Ignore]
		public void TestCustomSerializationLoop()
		{
			...
			using(FileStream stream = new FileStream("Prova.bin", FileMode.Create))
			{
				for (int i = 0; i < iterations; i++)
				{
					CustomSerializableObject ser = new
						CustomSerializableObject(i, 3.1415M, "Ciao Mondo!");

					CF.Serialize(stream, ser);
				}
			}
			sw.Stop();
			Console.WriteLine("Custom Serialization of {0} items took {1} milliseconds", iterations, sw.ElapsedMilliseconds);
			...
			using(FileStream stream2 = new FileStream("Prova.bin", FileMode.Open))
			{
				for (int i = 0; i < iterations; i++)
				{
					CustomSerializableObject ser2 = (CustomSerializableObject)
					                                CF.Deserialize(stream2);
					Assert.AreEqual(i, ser2.A);
					Assert.AreEqual(3.1415M, ser2.F);
					Assert.AreEqual("Ciao Mondo!", ser2.S);
				}
			}
			...
			sw.Stop();
			Console.WriteLine("Custom Deserialization of {0} items took {1} milliseconds", iterations, sw.ElapsedMilliseconds);
		}
